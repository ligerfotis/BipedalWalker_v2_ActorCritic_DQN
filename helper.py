...
class PreTrain():
    ...
    def run(self,pretrain_length):
        ...
        self.env.reset()
        # Take one random step to get the bipedal moving
        ...
        while(len(self.memory) < pretrain_length):
            ...
            next_state=next_state[:14]  #no lidar
            trwd +=reward 
            #bad rewards
...
'''
'''
#pre-train experiences
pretrain = PreTrain(env, score_requirement, memory_size)
pretrain.run(pretrain_length)
pretrain.save_experiences()
print(pretrain.load_experiences())
'''